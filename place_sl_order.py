...
qty = input("Enter Qty: ")
stoploss_price = input("Enter Stoploss Price: ")
...
place_order_details = {
    ...
    "tp": stoploss_price,  # Trigger price
...
if place_order_response.status_code == 200:
    ...
    if order_id:
        print(f"Stoploss Order placed successfully! SL Order ID: {order_id}")
    else:
        print("Stoploss Order ID not found in the response.")
else:
    print(f"Failed to place stoploss order. Status Code: {place_order_response.status_code}")
